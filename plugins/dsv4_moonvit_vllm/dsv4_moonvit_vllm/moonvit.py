    ...
    *,
    ...
    if tower_path is not None and Path(tower_path).is_file():
        try:
            ...
            meta["tower_missing"] = missing[:16]
            meta["tower_unexpected"] = unexpected[:16]
            msg = (
                f"Loaded MoonViT tower from {tower_path} "
                f"(missing={len(missing)} unexpected_file_keys={len(unexpected)})"
            )
            logger.info(msg)
            print(f"[dsv4_moonvit] {msg}", flush=True)
            if missing:
                sample = ", ".join(missing[:8])
                # Random residual params destroy color; fail closed.
                raise RuntimeError(
                    f"MoonViT tower incomplete after weight load: "
                    f"{len(missing)} missing params (e.g. {sample})"
                )
...
def _load_tower_weights(tower: nn.Module, path: str | Path) -> tuple[list[str], list[str]]:
    ...
    def _candidates(name: str) -> list[str]:
        # WebBrain vision_tower.safetensors keys already match vLLM MoonViT3d
        # (encoder.blocks.*.wqkv / .wo / .mlp.fc* / patch_embed.*).
        # Do NOT rename wqkv→attn.qkv_proj — that mapping is for other towers
        # and left our MoonViT weights randomly initialized (color-blind).
        bare = name.removeprefix("vision_tower.")
        outs = [
            name,
            bare,
            f"vision_tower.{bare}" if not name.startswith("vision_tower.") else name,
            # Legacy aliases only as last resort (older checkpoints / SGLang).
            bare.replace("wqkv.", "attn.qkv_proj.").replace("wo.", "attn.proj."),
            bare.replace("attn.qkv_proj.", "wqkv.").replace("attn.proj.", "wo."),
        ]
        seen: set[str] = set()
        ordered: list[str] = []
        for k in outs:
    ...
    # Non-persistent buffers (e.g. patch_embed.pos_emb.time_weight) are
    # computed at init and must not be required from the checkpoint.
    non_persistent: set[str] = set()
    for mod_name, module in tower.named_modules():
        # private API: _non_persistent_buffers_set
        nps = getattr(module, "_non_persistent_buffers_set", None) or set()
        for bname in nps:
            full = f"{mod_name}.{bname}" if mod_name else bname
            non_persistent.add(full)

    required = (set(params) | set(buffers)) - non_persistent
