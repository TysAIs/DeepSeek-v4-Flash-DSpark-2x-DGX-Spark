...
- **One-flag serve mode**: `ENABLE_VL_SIDECAR` defaults to **`0`** (text-only). `1` enables vision and sets main util — `0` → `GPU_MEMORY_UTILIZATION_TEXT` (**0.835**, larger KV), `1` → `GPU_MEMORY_UTILIZATION_VISION` (**0.80**) + VL sidecar. `start-deepseek-v4-flash-dspark.sh` exports `GPU_MEMORY_UTILIZATION` from the profile. Measured Available KV: text ~**18.08 GiB / ~2.49M** tokens; vision main **13.37 GiB / 1.37M** + VL **1.54 GiB / 84k**. Docs: `README.md` §Vision on/off.
- **VL sidecar 4-bit KV (GB10)**: production dtype is `VL_SIDECAR_KV_CACHE_DTYPE=int4_per_token_head` + `TRITON_ATTN` (≈½ fp8 KV bytes). True `--kv-cache-dtype nvfp4` is **blocked** on SM12.1 (FlashInfer requires SM100 trtllm-gen; Triton rejects `nvfp4`).
- **Live coexist profile (2026-08-11)**: main util **0.80**, `MAX_NUM_SEQS=4` → Available KV **13.37 GiB** / **1,374,417** tokens; VL `VL_SIDECAR_GPU_UTIL=0.04`, `VL_SIDECAR_MAX_MODEL_LEN=32768`, `VL_SIDECAR_MAX_NUM_SEQS=4` → Available KV **1.54 GiB** / **84,224** tokens (~2.6× of 32k). Util **0.03** can leave VL with too little KV under coexist; **0.05** also boots. ≥2M main KV with VL up still blocked by worker free memory. Docs: `README.md` §Vision, `results/vl-nvfp4-coexist-2026-08-11.md`.
- **pi / ZCode skill collision**: ZCode installer no longer copies `dspark-vision` into `~/.agents/skills` (pi already owns `~/.pi/agent/skills/dspark-vision` and warned on the duplicate).

### Added
- **VL sidecar TP=2**: `docker-compose.vl-sidecar.yml` now runs Qwen3-VL-4B with `--tensor-parallel-size 2 --nnodes 2` across head+worker (separate NCCL `VL_SIDECAR_MASTER_PORT=25100`). Per-GPU util co-tuned with 0731 (live **0.04**). Start is worker-first; prepare caches VL on both nodes; stop tears down VL on both ranks.
