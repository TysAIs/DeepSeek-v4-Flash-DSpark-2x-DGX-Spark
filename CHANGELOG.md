...
### Added
- **Multi-harness vision MCP install**: `scripts/install-dspark-vision-mcp.sh` + `plugins/dspark_vision_mcp/install_harnesses.py` detect and register `dspark-vision` into **pi**, **OMP**, **Hermes**, **opencode**, **goose**, **Grok Build**, and **OpenClaw** (`mcp.servers` in `~/.openclaw/openclaw.json`; [docs](https://docs2.openclaw.ai/tools/mcp)). Idempotent upsert + skill copy. `start-deepseek-v4-flash-dspark.sh` waits for the VL sidecar then runs the installer when `INSTALL_VISION_MCP` is on (defaults to follow `ENABLE_VL_SIDECAR`). Knobs documented in `.env.dspark.example` (`VISION_MCP_HARNESSES=auto`).
- **`plugins/dspark_vision_mcp`**: local vision MCP server (`describe_image` / `ocr_image` / `compare_images`) for the 0731 agent. Calls the Qwen3-VL sidecar on `:8889` with base64 data URIs (local paths + http(s), missing-file / sidecar-down errors, auto-downscale, max 4 images). Keeps vision fused into the 0731 conversation instead of switching models.
- **Local VL sidecar for vision** (replaces native MoonViT as the production vision path): `docker-compose.vl-sidecar.yml` serves `Qwen/Qwen3-VL-4B-Instruct-FP8` head-only on `:8889` (served name `qwen3-vl-4b`); `start-deepseek-v4-flash-dspark.sh` brings it up when `ENABLE_VL_SIDECAR=1` (set in `.env.dspark`). 0731 serve is text-only again (`DSPARK_MODEL=drowzeys/keys-...Abliterated-32-32`, `SERVED_MODEL_NAME=deepseek-v4-flash-0731`, `GPU_MEMORY_UTILIZATION` tuned in `.env.dspark` to leave room for the sidecar). `scripts/vision-reason.py` now extracts via the sidecar and reasons via 0731 at max effort.
