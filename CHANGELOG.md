...
- **Multi-harness vision MCP install**: `scripts/install-dspark-vision-mcp.sh` + `plugins/dspark_vision_mcp/install_harnesses.py` detect and register `dspark-vision` into **pi**, **OMP**, **Hermes**, **opencode**, **goose**, **Grok Build**, and **OpenClaw**. Idempotent upsert + skill copy. `start-deepseek-v4-flash-dspark.sh` waits for the VL sidecar then runs the installer when `INSTALL_VISION_MCP` is on (defaults to follow `ENABLE_VL_SIDECAR`). Knobs in `.env.dspark.example` (`VISION_MCP_HARNESSES=auto`).
