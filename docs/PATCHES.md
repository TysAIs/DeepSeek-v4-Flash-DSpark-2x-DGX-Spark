...
## Patch 3 — SM120 sparse-MLA decode workspace size

### Symptom
Startup died during kernel warmup (and small decode batches at runtime would hit
the same path):

```
AttributeError: 'NoneType' object has no attribute 'shape'
  flashinfer/mla/_sparse_mla_sm120.py:146  _check_last_dim_512(mid_out, ...)
  vllm/models/deepseek_v4/nvidia/flashinfer_sparse.py:867  _forward_decode
```

### Root cause
For decode batches of ≤64 tokens, `_forward_decode` routes to FlashInfer's split-K
kernel and carves `mid_out`/`mid_lse` out of a fixed 128 MiB workspace via
`_sparse_mla_decode_workspace`. The split count is derived from the **padded**
index widths: `num_splits = ceil(swa_topk/64) + ceil(extra_topk/64)` with
`extra_topk` = c128a topk-indices buffer width = `max_num_batched_tokens` (8192),
not the actual valid top-k. Worst case (64 tokens × 32 heads × 130 splits ×
512 d_v bf16 + fp32 LSE) needs ~262 MiB; at 32 tokens it already needs
136,847,360 B > 134,217,728 B. On overflow the helper returns `(None, None)` and
the kernel wrapper dereferences `mid_out.shape` → the AttributeError above.

### Fix
`patches/flashinfer-dsv4-workspace-512mb.patch`: raise
`_FLASHINFER_DSV4_WORKSPACE_BUFFER_SIZE` in
`vllm/models/deepseek_v4/nvidia/flashinfer_sparse.py` from 128 MiB to 512 MiB
(per device). Applied in-image via `docker create/cp/commit` on both nodes; costs
an extra 384 MiB per GPU, allocated lazily at first decode.

### Note
The cleaner upstream fix is deriving the split count from the real per-row top-k
lengths (≤ `index_topk`=512) instead of the padded buffer width; the constant bump
is the zero-risk stopgap.
